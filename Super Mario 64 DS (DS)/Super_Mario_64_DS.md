...
That closes the tree table's fifth entry: `$9C10` = **ar1.narc member 16**, whose only material is named `mat_main_tree` — the castle-grounds tree, a two-triangle billboard like its filesystem siblings. The same archives hold the collectibles, self-identified by their material names: arc0 member 5 is the **coin** (materials `coin`/`coin_a`; member 7 the red variant), member 21 the **power star** (`mat_body`/`mat_eye` with embedded `star` strings), member 25 the **silver star** (`mat_star_silver`). The coin and star actors' load paths have not been code-traced yet — the models are bound to their placements on this in-cartridge naming evidence.

## 5. The model transform — traced so far, and what is still open

Placement data is fully traced: the position shorts are fx world coordinates (`LSL #12` in the spawners at `$020FE8AC`/`$020FE960`), rotations are DS angle-index units, and the object→actor and model bindings above all come from the game's own tables and code. What is *not* yet traced end-to-end is the **render transform** — how the engine maps a model's authored fx vertices and an actor's world position onto the screen:

* The engine object carries a scale vector at `+$80` (fx12), initialised to 1.0 — the `MOV #$1000` triple-store idiom appears across the actor code — and **none of the placed actors write it statically**; actors like the super mushroom size themselves at runtime.
* The map actor (actor 335, create `$020FB8BC`, spawned by the level processor together with actor 334) builds the visible level in its init method at `$020F9E98`: it reads the settings table for the current level and creates the stage's render objects through `$0202199C`, which passes **an explicit 1.0 (`#$1000`) scale pair** into the generic model-object creator at `$02020994`. That creator — and the draw code below it — is where the transform trace currently stands.

Until that trace lands, the viewer uses one documented *export convention*, built only from traced units: the exported GLBs bake each model's header 2^shift onto its raw fx vertices, placements map by `short × 2^stageShift / 4096`, and object instances display at their **authored fx size** (the bake reversed, scale `2^-shift`) — the only size the cartridge states for them. Runtime-scaled actors therefore render at authored size, deliberately incomplete rather than calibrated. The two-triangle billboard models (the trees, the super mushroom) additionally carry GX texture coordinates that overflow their texture in texel space (the tree quad spans `t = -14.75..64` on a 64-texel texture); the engine's billboard draw path — also still untraced — evidently remaps them, and the exporter normalises the UV range to the texture as the corresponding display convention.
