...
## 5. Calibration — and what is still open

Three numbers tie placements to the exported GLBs. Positions: a stage GLB stores raw fx floats times the stage's 2^shift, and a placement short (`LSL #12` into fx by the spawner) is `short/4096` fx — so stage-GLB position = `short × 2^stageShift / 4096`. (A `/4096`-only first pass passed a bounding-box containment test — 99.9% of placements inside — but clustered everything at half radius on shift-1 stages like Bob-omb Battlefield; the containment metric cannot catch an under-scale.) Object size: the engine's per-object scale field (object `+$80`, an fx12 vector initialised to 1.0 by the `MOV #$1000` idiom across the actor code) is *not* statically written by any of the placed actors, and the collectibles are sized by their actors at runtime — the super mushroom literally grows — so the viewer uses a calibrated **1/16-of-world** ratio (the size the tree's shift-4 bake cancels out to, and the object ratio Mario Kart DS ships) with per-model corrections for the large-authored archive models. Tracing the render path's actual object scaling is the main open item in this part.
