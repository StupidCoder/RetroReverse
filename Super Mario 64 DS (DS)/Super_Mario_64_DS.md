...
**There is no stake.** The famous wooden post the chomp is tethered to in the Nintendo 64 original — a separate poundable object there — does not exist in the remake, even though the *"See the red coin on top of the stake?"* signpost text survived the port. The question came up because Bob-omb Battlefield's viewer shows pile posts (actor 27, `pile.bmd`) elsewhere but nothing at the chomp's anchor, and the answer is exhaustive: the placed chomp is `daWanwan_c` (actor 219) in **overlay 14 — Bob-omb Battlefield's own level overlay** (each level overlay carries its level-specific actors: here also `daObjWanwanShutter_c`, the gate). Its init acquires exactly four files — `ar1_2` body, `ar1_1` chain, `ar1_3`/`ar1_4` clips — its draw (`$02112994`) renders the body wrapper plus **seven** chain-link wrappers and nothing more, the overlay makes no actor-factory calls at all (the chomp spawns no children; run in the actor oracle through create, init and twelve stepped frames it loads nothing further), the only actor it looks up is its gate (`$02112A44`: find-by-ID 41), the stage mesh at the anchor is plain `mat_grass`, and the level `.kcl` there is flat floor at the placement height (the game's own down-ray walker reports y=750, same as the open field). The anchor is bare ground; the red coin floats where the N64 stake stood, and the pound-to-free bookkeeping lives inside the chomp object itself (the `wanwan2` init plants a count of 3 at `+$6CC`). The four `pile` posts are an unrelated 2×2 group with their own placements and their own scaled-down `pile.kcl` colliders.
