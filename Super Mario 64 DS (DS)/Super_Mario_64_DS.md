...
The static trace (`extract/sm64ds/actors.go`, report via `cmd/actortrace`) binds ~24 placed actors this way — trees, signboards, mushrooms, switches, sea-weed, push-blocks and the level-local set-dressing.

The **enemy banks** extend the same trace to the creatures. Overlays 60–102 are per-enemy code banks: each carries actor profiles (create function at `+$00`, u16 actor ID at `+$04`) — some with C++ typeinfo (`daKrb_c` the goomba in overlay 84, `daBmb_c` the bob-omb in 102, `daBombking_c` in 78, `daRedBombhei_c`, the piranhas, the snowmen…) — and registers its model files in its static initialisers, exactly like the engine actors. Two subtleties: the second u16 of a bank profile is *not* a duplicate of the ID (the goomba carries `$1A` there — it is a priority/order field), and **the ARM9 profile array itself points into bank address space** for these actors — actor 200's entry (`$021308EC`) lands three records before the goomba's in overlay 84, and is only meaningful while that bank is loaded. Since several banks share each RAM slot, the same actor ID can resolve into different banks; the trace binds only IDs whose resolution is unambiguous across all banks and drops the rest (`TraceBankActorModels`). That turns the placed enemies into model bindings — on Bob-omb Battlefield alone: 21 goomba spawners (`kuribo_model`, actors 200/201 — the goomba trio records adjacent to `daKrb_c`), 11 bob-ombs, the red bob-omb buddies, King Bob-omb, the cannons (`houdai`) and the water bombs — 3,223 of 4,350 placements bound overall. A handful of actors whose nearest-literal scan lands on a neighbouring unit's prop (the chain chomp picking up a water plane, the enemy-spawner `daECreate_c`) are blacklisted rather than shown wrong.
...
The viewer replicates what is traced: coins spin at the engine's `$C00`-per-frame rate, and clicking a signpost pops the traced interaction (the real dialog needs the message archives). Goombas stay markers until `.bca` lands.
