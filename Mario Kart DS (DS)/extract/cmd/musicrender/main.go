// musicrender renders Mario Kart DS's sequenced music from the cartridge's
...
// pipeline as the other games' music (Part VI).
//
...
import (
	...
	"retroreverse.com/tools/nds/sdat"
)

const rate = 32768 // the DS mixer's output rate

func main() {
	sdatPath := flag.String("sdat", "../extracted/files/data/Sound/sound_data.sdat", "SDAT file")
	...
	for i := from; i < to; i++ {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "  seq %02d: %v\n", i, err)
			continue
		}
		if len(L) < rate { // sub-second sequences are jingles/stingers cut short
			fmt.Printf("  seq %02d: %5.1fs (skipped, too short)\n", i, float64(len(L))/rate)
			continue
		}
		fadeOut(L, R)
		wav := filepath.Join(*outDir, fmt.Sprintf("seq_%02d.wav", i))
		if err := writeWAV(wav, L, R); err != nil {
			die(err)
		}
		mp3 := filepath.Join(*outDir, fmt.Sprintf("seq_%02d.mp3", i))
		...
		tracks = append(tracks, trackEntry{
			...
			URL:  fmt.Sprintf("public/mariokart/music/seq_%02d.mp3", i),
		})
		fmt.Printf("  seq %02d: %5.1fs → %s\n", i, float64(len(L))/rate, filepath.Base(mp3))
	}
	if *seqN < 0 {
