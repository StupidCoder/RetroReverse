...
export async function mount(ctx, doc) {
  ...
  const ar = new MapAR({
    ...
    model: () => ({
      ...
      atlasImg,
      tickHz,
      // Non-wrapping maps clip their object layer to the playfield.
      clip: tm.wrap === 'x' ? null : { w: map.widthPx, h: map.heightPx },
      placements: pickables.map(({ inst, pl, obj }) => ({
        inst, pl, obj, visible: () => visibleThrough(inst.node),
      })),
      // The session drives the placement clock itself — window rAF is not
      // reliably serviced while presenting — and leaves the map effects alone,
      // since the AR renderer reproduces those from the document instead of
      // from repainted canvases.
      advance: (dt) => { const df = dt * tickHz; for (const a of anims) a.tick(df); },
      begin: () => app.ticker.stop(),
      end: () => app.ticker.start(),
    }),
  });
