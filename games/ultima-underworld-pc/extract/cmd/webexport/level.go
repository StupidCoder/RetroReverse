...
// level.go holds the level-decode pipeline, copied from cmd/levexport: it builds
// one dungeon level's self-contained textured mesh (floors/walls/ceilings with
// real W64.TR/F32.TR textures), bakes the 3-D-class objects (doors, pillars,
// bridges) into the mesh, and collects the billboard sprites, directional
// creatures and click targets. cmd/webexport reshapes this intermediate outMesh
// into the format-2 asset tree; cmd/levexport keeps its own self-contained JSON.
...
import (
	...
	"retroreverse.com/games/ultima-underworld-pc/extract/strpak"
...
// buildLevel decodes dungeon level `level` (0-based) from the installed game dir
// into the intermediate outMesh: the textured level geometry plus baked 3-D
// objects, billboard sprites, directional creatures and click targets. It mirrors
// cmd/levexport's main(), minus the JSON write.
func buildLevel(game string, level, palN int, ceil bool) *outMesh {
