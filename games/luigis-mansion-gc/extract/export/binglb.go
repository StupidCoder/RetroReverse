...
// BinBounds returns the world-space AABB of a .bin model — the same node
// walk BinGLB bakes, positions only (for room storey assignment and camera
// derivation).
func BinBounds(m *lm.Bin) (min, max [3]float32, ok bool) {
	world := make([]lm.Mtx34, len(m.Nodes))
	for i := range world {
		world[i] = lm.Identity34
	}
		...
		for _, pair := range m.Nodes[ni].Pairs {
			...
			for _, p := range dl {
				for _, v := range p.Verts {
					...
					w := world[ni].Apply(m.Positions[v.Pos])
					if first {
...
func BinExport(src *Source, spec, out string) error {
