// music.go is the music stage (folded from ex-cmd/musicrender): it synthesises
// Elite's docking theme ENTIRELY from the decrypted game image — a
// reimplementation of the $BDDC music sequencer (Elite.md Part VI) driving the
// shared SID emulator (tools/platform/c64/sid). No emulator capture is involved;
// the only inputs are the music command stream at $C034 in memory_final.bin and
// the opcode behaviour read from the disassembly.
//
// The engine is a nibble-packed bytecode: a 16-bit play pointer walks the stream,
// opcodes are 4-bit (two per byte, low nibble first), and each sets SID registers
// (notes, ADSR, pulse width, waveform, filter) or steps time. A note step holds
// for the default length ($BDDB) frames; voices 2 and 3 alternate between their
// pitch and a +$20-detuned copy (5- and 6-frame cycles) — the chorused Elite sound
// — and the gates release two frames before each step ends. The player ticks once
// per PAL video frame (~50.12 Hz). Rendering stops at the stream's loop opcode
// (one pass). ffmpeg (libmp3lame) encodes the MP3.
package main
...
const (
	...
	musicAddr = 0xC034 // the music command stream ($C034)
	maxSec    = 240.0
)
...
// w writes a SID register (offset from $D400).
func (p *player) w(reg int, v uint8) { p.chip.Write(uint8(reg), v) }

// gate retriggers a voice's gate with the current waveform (regs $04/$0B/$12).
...
func (p *player) runCommands() {
	for {
		...
		case 7: // set ADSR: all attack/decays, then all sustain/releases
...
func exportMusic(inDir, outDir string) []AssetIndex {
	...
	p := &player{
		mem:  mem,
		ptr:  musicAddr, // reader pre-increments, so the first byte read is musicAddr+1
		dur:  8,
