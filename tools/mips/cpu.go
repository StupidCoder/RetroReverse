...
// Coprocessor2 is the GTE seen by the CPU: 32 data registers and 32 control
// registers reached by mfc2/mtc2/cfc2/ctc2 and lwc2/swc2, plus the command
// stream (cop2). tools/mips/gte.go implements it; a nil GTE degrades gracefully
// (reads return 0, writes and commands are dropped) so a boot trace does not
// stall before the GTE exists.
type Coprocessor2 interface {
	Read(reg uint32) uint32       // mfc2 / swc2 source: data register
	Write(reg uint32, v uint32)   // mtc2 / lwc2 dest: data register
	ReadCtrl(reg uint32) uint32   // cfc2: control register
	WriteCtrl(reg uint32, v uint32) // ctc2: control register
	Command(op uint32)            // cop2: 25-bit command
}
...
type CPU struct {
	...
	GTE Coprocessor2 // optional COP2 (see gte.go)

	// Syscall optionally handles the `syscall` instruction (return true if
	// serviced, so the core does not raise the Sys exception). The PSX BIOS-HLE
	// intercepts A0/B0/C0 calls at their entry addresses instead, so this is
	// usually left nil and `syscall` vectors through the exception handler.
...
func (c *CPU) Reset() {
	...
	c.COP0 = [32]uint32{}
	c.COP0[cop0PRId] = 0x00000002 // R3000A revision, as the PSX reports
	c.ld = loadSlot{}
