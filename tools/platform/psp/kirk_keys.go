...
// kirk_keys.go holds the PSP platform crypto constants used by the KIRK engine
// (kirk.go) to decrypt a ~PSP executable. These are hardware constants of the PSP's
// KIRK security coprocessor — not on any UMD and not derivable from ciphertext — so
// they are baked here as documented platform constants (the same standing as the
// N64 CIC seed), with provenance: the KIRK CMD4/7 key vault and the CMD1 master AES
// key are the PSP's fixed keys; keyEBOOT2xx is the XOR seed selected by the ~PSP
// header tag 0xC0CB167C (retail 2.xx game EBOOT.BIN). The KIRK *algorithm* is
// reimplemented from the format in kirk.go; only these constants are transcribed.

// keyvault: KIRK CMD4/7 key table (0x80 entries x 16 bytes)
var keyvault = [0x80][16]byte{
...
var kirk1Key = [16]byte{0x98, 0xC9, 0x40, 0x97, 0x5C, 0x1D, 0x10, 0xE8, 0x7F, 0xE6, 0x0E, 0xA3, 0xFD, 0x03, 0xA8, 0xBA}

// g_keyEBOOT2xx: XOR seed for the ~PSP tag 0xC0CB167C (2.xx EBOOT). 0x90 bytes.
var keyEBOOT2xx = [0x90]byte{
...
// g_keyEBOOT1xx: XOR seed for the ~PSP tag 0x08000000 (1.xx-firmware EBOOT,
// kirk7 key id 0x4B). 0x90 bytes. Documented platform constant, transcribed
// from the PSP KIRK/PRXDecrypter specification (little-endian bytes of the
// reference's u32 array); verified by the SHA-1 header check and the plaintext
// ELF it yields.
var keyEBOOT1xx = [0x90]byte{
