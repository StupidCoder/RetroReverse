...
func TestPerspUVOrthoReducesToAffine(t *testing.T) {
	v0 := screenVertex{u: 0, v: 0, invW: 1}
	v1 := screenVertex{u: 1, v: 0, invW: 1}
	v2 := screenVertex{u: 0, v: 1, invW: 1}

	for _, b := range [][3]float32{
		{1, 0, 0}, {0, 1, 0}, {0, 0, 1},
		{0.5, 0.5, 0}, {1.0 / 3, 1.0 / 3, 1.0 / 3}, {0.25, 0.25, 0.5},
	} {
		u, v := perspUV(b[0], b[1], b[2], v0, v1, v2)
		wantU := b[0]*v0.u + b[1]*v1.u + b[2]*v2.u
		wantV := b[0]*v0.v + b[1]*v1.v + b[2]*v2.v
		if u != wantU || v != wantV {
...
func TestPerspUVAtVertices(t *testing.T) {
	v0 := screenVertex{u: 0.1, v: 0.2, invW: 1}
	v1 := screenVertex{u: 0.7, v: 0.9, invW: 1.0 / 50}
	v2 := screenVertex{u: 0.4, v: 0.3, invW: 1.0 / 1000}
	...
	for i, c := range []struct {
		...
		u, v := perspUV(c.b[0], c.b[1], c.b[2], v0, v1, v2)
		if math.Abs(float64(u-c.vx.u)) > 1e-6 || math.Abs(float64(v-c.vx.v)) > 1e-6 {
			t.Errorf("vertex %d: got (%v,%v), want (%v,%v)", i, u, v, c.vx.u, c.vx.v)
		}
	}
}
...
func TestPerspUVRecedingEdge(t *testing.T) {
	near := screenVertex{u: 0, invW: 1.0 / 1}
	far := screenVertex{u: 1, invW: 1.0 / 3}
	unused := screenVertex{u: 0, invW: 1}

	u, _ := perspUV(0.5, 0.5, 0, near, far, unused)
...
func TestPerspUVMatchesGroundTruthAlongAnEdge(t *testing.T) {
	...
	v0 := screenVertex{u: 0, v: 3, invW: 1 / w0}
	v1 := screenVertex{u: 1, v: 7, invW: 1 / w1}
	v2 := screenVertex{u: 9, v: 9, invW: 1 / w0}

	for i := 0; i <= 10; i++ {
		s := float32(i) / 10
		gotU, gotV := perspUV(1-s, s, 0, v0, v1, v2)

		num := s / w1
		den := (1-s)/w0 + s/w1
		tt := num / den
		wantU := v0.u + (v1.u-v0.u)*tt
		wantV := v0.v + (v1.v-v0.v)*tt
