...
func (g *gpu) drawPrimitive(m *Machine, prim uint32, vat, vsize int, data []byte) {
	...
	if drawTrace && count > 0 {
		...
		fmt.Fprintf(os.Stderr, "DRAW prim 0x%02X vat %d n %d  v0 (%.1f,%.1f,z%.0f)%s rgba %d,%d,%d,%d ntc=%d uv (%.2f,%.2f)  tex0 0x%06X fmt%X %dx%d  px w=%d zrej=%d arej=%d  stages=%d cull=%d bp41=%06X af=%06X zm=%02X a0=%.0f ka0=%.0f vcd=%03X mat0=%08X amb0=%08X cc0=%08X ca0=%08X tev0=%06X/%06X\n",
			...
			g.XFMem[0x100E], g.XFMem[0x1010], g.BP[0xC0], g.BP[0xC1])
