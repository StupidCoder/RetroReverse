...
// rcprom.go holds the two lookup tables burnt into the RSP's silicon, which the
// reciprocal and reciprocal-square-root instructions index.
//
// These are not on the cartridge, and they are not derived from it. They are a
// property of the chip, exactly as the CIC seed IPL3 checks itself against is a
// property of the boot chip — hardware facts that the medium cannot supply and
// the oracle must. They are transcribed from Nintendo 64 hardware documentation,
// not from any source specific to a game.
//
// They defeat a closed form. The obvious candidate, floor(65536*(512-i)/(512+i)),
// reproduces entries 0 and 1 (0xFFFF, 0xFF00) and then diverges at entry 2,
// where it yields 0xFE02 against the chip's 0xFE01. Computing the table would
// therefore have produced geometry that is very slightly wrong everywhere, which
// is the hardest kind of error to notice and the hardest to chase.

// rcpROM approximates 1/x for a mantissa in [1, 2), indexed by its top nine bits.
...
// rsqROM approximates 1/sqrt(x), indexed by the low bit of the input's scale
// followed by the mantissa's top eight bits.
