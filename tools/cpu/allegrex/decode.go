...
func Decode(code []byte, addr uint32) Inst {
	...
	case 0x11: // COP1 (FPU)
		return decodeCop1(in, w, rs, rt, rd, shamt)
	case 0x12: // COP2 (VFPU register moves / control)
		return decodeCop2(in, w, rs, rt, rd)

	case 0x14, 0x15, 0x16, 0x17: // branch-likely (beql/bnel/blezl/bgtzl)
		in.Flow, in.Target, in.HasTarget, in.HasDelay = FlowBranch, branchT, true, true
		m := [...]string{0x14: "beql", 0x15: "bnel", 0x16: "blezl", 0x17: "bgtzl"}[op]
		if op == 0x14 || op == 0x15 {
			set(m, fmt.Sprintf("%s %s, %s, $%08X", m, reg(rs), reg(rt), branchT))
		} else {
			set(m, fmt.Sprintf("%s %s, $%08X", m, reg(rs), branchT))
		}

	case 0x1C: // SPECIAL2 (MIPS32R2: mul/madd/clz/clo)
		return decodeSpecial2(in, w, rs, rt, rd, funct)
	case 0x1F: // SPECIAL3 (MIPS32R2: ext/ins/bshfl)
		return decodeSpecial3(in, w, rs, rt, rd, shamt, funct)
	...
	case 0x31: // lwc1 — load word to FPU register
		set("lwc1", fmt.Sprintf("lwc1 $f%d, %d(%s)", rt, simm, reg(rs)))
	case 0x39: // swc1 — store FPU register
		set("swc1", fmt.Sprintf("swc1 $f%d, %d(%s)", rt, simm, reg(rs)))
	case 0x32, 0x36, 0x35, 0x3A, 0x3E, 0x3D, 0x34, 0x37, 0x3C, 0x18, 0x19, 0x1B:
		return decodeVFPU(in, w, op, rs, rt, uint32(simm))
...
// decodeSpecial2 handles the MIPS32R2 SPECIAL2 group (op 0x1C).
func decodeSpecial2(in Inst, w, rs, rt, rd, funct uint32) Inst {
	set := func(mnem, text string) Inst { in.Mnem, in.Text = mnem, text; return in }
	switch funct {
	case 0x00:
		return set("madd", fmt.Sprintf("madd %s, %s", reg(rs), reg(rt)))
	case 0x01:
		return set("maddu", fmt.Sprintf("maddu %s, %s", reg(rs), reg(rt)))
	case 0x02:
		return set("mul", fmt.Sprintf("mul %s, %s, %s", reg(rd), reg(rs), reg(rt)))
	case 0x04:
		return set("msub", fmt.Sprintf("msub %s, %s", reg(rs), reg(rt)))
	case 0x05:
		return set("msubu", fmt.Sprintf("msubu %s, %s", reg(rs), reg(rt)))
	case 0x20:
...
// decodeSpecial3 handles the MIPS32R2 SPECIAL3 group (op 0x1F): the bitfield ops
// ext/ins and the BSHFL sub-group (wsbh/seb/seh/bitrev, discriminated by shamt).
func decodeSpecial3(in Inst, w, rs, rt, rd, shamt, funct uint32) Inst {
	set := func(mnem, text string) Inst { in.Mnem, in.Text = mnem, text; return in }
	switch funct {
	case 0x00: // ext rt, rs, pos, size
		return set("ext", fmt.Sprintf("ext %s, %s, %d, %d", reg(rt), reg(rs), shamt, rd+1))
	case 0x04: // ins rt, rs, pos, size
		return set("ins", fmt.Sprintf("ins %s, %s, %d, %d", reg(rt), reg(rs), shamt, rd-shamt+1))
	case 0x20: // BSHFL
		switch shamt {
		case 0x02:
			return set("wsbh", fmt.Sprintf("wsbh %s, %s", reg(rd), reg(rt)))
		case 0x10:
			return set("seb", fmt.Sprintf("seb %s, %s", reg(rd), reg(rt)))
		case 0x18:
			return set("seh", fmt.Sprintf("seh %s, %s", reg(rd), reg(rt)))
		case 0x14:
			return set("bitrev", fmt.Sprintf("bitrev %s, %s", reg(rd), reg(rt)))
		}
	}
	return word(in, w)
}
...
func decodeSpecial(in Inst, w, rs, rt, rd, shamt, funct uint32) Inst {
	...
	case 0x02:
		if rs&1 != 0 { // rotr (MIPS32R2): srl encoding with the rotate bit set
			return set("rotr", fmt.Sprintf("rotr %s, %s, %d", reg(rd), reg(rt), shamt))
		}
	...
	case 0x06:
		if shamt&1 != 0 { // rotrv (MIPS32R2): srlv encoding with the rotate bit set
			return set("rotrv", fmt.Sprintf("rotrv %s, %s, %s", reg(rd), reg(rt), reg(rs)))
		}
	...
	case 0x0A:
		return set("movz", fmt.Sprintf("movz %s, %s, %s", reg(rd), reg(rs), reg(rt)))
	case 0x0B:
		return set("movn", fmt.Sprintf("movn %s, %s, %s", reg(rd), reg(rs), reg(rt)))
	case 0x0D:
		in.Flow = FlowStop
