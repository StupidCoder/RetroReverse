// codetracemips is a recursive-descent ("code-tracing") disassembler for MIPS
...
func main() {
	...
	flag.Parse()
	if flag.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: codetracemips [-base A] [-skip N] -entry A,B,C [-table ADDR:N] [-annotate F] [-o out] image.bin")
		os.Exit(2)
	}
	if err := run(flag.Arg(0), *base, *skip, *entry, tables, *annotate, *out); err != nil {
		fmt.Fprintln(os.Stderr, "codetracemips:", err)
		os.Exit(1)
	}
}
...
func trace(mem []byte, base uint32, seeds []uint32) *traced {
	t := &traced{instr: map[uint32]mips.Inst{}, covered: make([]bool, len(mem)), callers: map[uint32]int{}}
	...
	decodeAt := func(pc uint32) mips.Inst {
		if in, ok := t.instr[pc]; ok {
			return in
		}
		in := mips.Decode(mem[pc-base:], pc)
		t.instr[pc] = in
		cover(pc, in.Len)
		return in
	}
