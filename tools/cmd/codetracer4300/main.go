// codetracer4300 is a recursive-descent ("code-tracing") disassembler for NEC
// VR4300 (Nintendo 64) code, the counterpart of codetracemips / codetracearm.
...
// bnel, and the REGIMM and COP1 variants), whose delay slot executes only when
// the branch is taken — it is still reachable code, so it is still covered, but
// the fall-through path resumes after it exactly as for an ordinary branch.
//
...
// Usage:
//
//	codetracer4300 [-base ADDR] [-skip N] -entry A,B,C [-table ADDR:N] [-annotate FILE] [-o out] image.z64
//
// The image is loaded flat at -base (default 0); -skip drops that many leading
// file bytes. A cartridge image in any byte order is normalised to big-endian.
...
func main() {
	...
	flag.Parse()
	if flag.NArg() != 1 {
		fmt.Fprintln(os.Stderr, "usage: codetracer4300 [-base A] [-skip N] -entry A,B,C [-table ADDR:N] [-annotate F] [-o out] image.z64")
		os.Exit(2)
	}
	if err := run(flag.Arg(0), *base, *skip, *entry, tables, *annotate, *out); err != nil {
		fmt.Fprintln(os.Stderr, "codetracer4300:", err)
		os.Exit(1)
	}
}
...
func run(path, baseS string, skip int, entryStr string, tables multiFlag, annPath, outPath string) error {
	...
	if order, ok := n64.Normalize(raw); ok && order != n64.OrderZ64 {
		fmt.Fprintf(os.Stderr, "codetracer4300: image stored as %s; normalised to big-endian\n", order)
	}
	base, err := hx(baseS)
	if err != nil {
	...
	// Pointer tables hold big-endian words, like everything else on the cartridge.
	rd32 := func(a uint32) uint32 {
		o := int(a - base)
		if o < 0 || o+4 > len(mem) {
			return 0
		}
		return binary.BigEndian.Uint32(mem[o:])
	}
...
func trace(mem []byte, base uint32, seeds []uint32) *traced {
	t := &traced{instr: map[uint32]r4300.Inst{}, covered: make([]bool, len(mem)), callers: map[uint32]int{}}
	...
	decodeAt := func(pc uint32) r4300.Inst {
		if in, ok := t.instr[pc]; ok {
			return in
		}
		in := r4300.Decode(mem[pc-base:], pc)
		t.instr[pc] = in
		cover(pc, in.Len)
		return in
	}
	...
	for len(work) > 0 {
		...
		for {
			...
			// cover it before acting on the control transfer. A branch-likely's
			// slot runs only on the taken path, but it is code either way.
			if in.HasDelay && inRange(pc+4) {
				decodeAt(pc + 4)
			}
			switch in.Flow {
			case r4300.FlowBranch:
				push(in.Target)
