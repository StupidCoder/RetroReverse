// disallegrex linearly disassembles Allegrex (Sony PSP, MIPS32R2) code from a raw binary
// — an extracted PSP module text image, a memory dump, or a slice of one. You tell
// it the CPU address the first decoded byte maps to; all numbers are hex.
//
//	disallegrex [-base ADDR] [-skip N] [-start ADDR] [-end ADDR] file.bin
//
...
// jumps or separate code from data — use codetraceallegrex for that. To disassemble
// a PSP module, skip its 0x800 header with -skip 2048 and set -base to t_addr.
package main
...
func die(format string, a ...interface{}) {
	fmt.Fprintf(os.Stderr, "disallegrex: "+format+"\n", a...)
	os.Exit(2)
}
...
func main() {
	...
	flag.Parse()
	if flag.NArg() != 1 {
		die("usage: disallegrex [-base A] [-skip N] [-start A] [-end A] file.bin")
	}
	...
	for _, l := range allegrex.Disassemble(code[start-base:end-base], uint32(start)) {
		fmt.Println(l)
	}
}
